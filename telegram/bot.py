...
def send_composition_to_telegram(form):
    ...
    for field in form:
        if str(field).split()[-1] == 'checked>':
            ingredients += str(field.label_tag().split('>')[1].split('<')[0]).strip(':'),
        elif str(field).split()[1].split('"')[1] == 'text':
            message += str(field).strip('<>').split('value=')[1].split('maxlength=')[0] + '*'
    ...
    bot_message(f'''У тебе новий запит на створення композиції: 
{name}
{phone}
